...
int main()
{
    ...
    { //エラー処理
        ...
        std::cerr << "Error socket:" << errno; //標準出力
        exit(1);                               //異常終了
    }

    //アドレスの生成
    struct sockaddr_in addr = *(new sockaddr_in);  //接続先の情報用の構造体(ipv4)
    ...
    { //ソケット, アドレスポインタ, アドレスサイズ //エラー処理
        ...
        std::cerr << "Error bind:" << errno; //標準出力
        exit(1);                             //異常終了
    }
    ...
    { //ソケット, キューの最大長 //エラー処理
        ...
        std::cerr << "Error listen:" << errno; //標準出力
    ...
    { //エラー処理
        ...
        std::cerr << "Error accept:" << errno; //標準出力
        exit(1);                               //異常終了
    }
    ...
    {
        //受信
        char str[256];                               //受信用データ格納用
        recv(connect, str, 256, 0);                  //受信
        std::cout << "message:" << str << std::endl; //標準出力

        //送信
        send(connect, str, 256, 0);    //送信
